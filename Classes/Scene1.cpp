...
void Scene1::Gameover()
{
	...
	guojing->stopAllActions();
	_eventDispatcher->removeAllEventListeners();
	unschedule(schedule_selector(Scene1::update));
	unschedule(schedule_selector(Scene1::updateGuojing));
	auto visibleSize = Director::getInstance()->getVisibleSize();
	Vec2 origin = Director::getInstance()->getVisibleOrigin();
	bg = win ? Sprite::create("Scene1_CG/ge0.jpg") : Sprite::create("Scene1_CG/be0.jpg");
	bg->setPosition(origin.x + visibleSize.width / 2, origin.y + visibleSize.height / 2);
...
void Scene1::onTouchEnded(Touch *touch, Event *event) {
	...
	if (win&&num <= 2) {
		char str[30];
		auto visibleSize = Director::getInstance()->getVisibleSize();
		sprintf(str, "Scene1_CG/ge%d.jpg", num);
		auto temp = bg;
		temp->removeFromParentAndCleanup(true);
		...
		char str[30];
		auto visibleSize = Director::getInstance()->getVisibleSize();
		sprintf(str, "Scene1_CG/be%d.jpg", num);
		auto temp = bg;
		temp->removeFromParentAndCleanup(true);
		bg = Sprite::create(str);
		bg->setPosition(visibleSize.width / 2, visibleSize.height / 2);
